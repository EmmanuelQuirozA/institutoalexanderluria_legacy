<?php
    ...
    $stmt = $connection->prepare("SELECT COUNT(*) AS allcount FROM usuarios INNER JOIN roles ON usuarios.idRol=roles.idRol INNER JOIN personas ON usuarios.idPersona=personas.idPersona");
    $stmt->execute();             
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

    ## Total number of records with filtering
    $stmt = $connection->prepare("SELECT COUNT(*) AS allcount FROM usuarios INNER JOIN roles ON usuarios.idRol=roles.idRol INNER JOIN personas ON usuarios.idPersona=personas.idPersona WHERE 1 ".$searchQuery);
