<?php
	...
	$searchByestatusPago = $_POST["searchByestatusPago"];
	...
	$searchByusernameAprobo = $_POST["searchByusernameAprobo"];
	$searchByfechaAprobacion = $_POST["searchByfechaAprobacion"];
	...
	$columns = array(
		...
		"usuariosRegistro.username"=>$searchByusername,
		...
		"estatusPago"=>$searchByestatusPago,
		"cicloEscolar"=>$searchBycicloEscolar,
		"mesColegiatura"=>$searchBymesColegiatura,
		"observaciones"=>$searchByobservaciones,
		"folio"=>$searchByfolio,
		"usuariosAprobo.username"=>$searchByusernameAprobo,
		"fechaAprobacion"=>$searchByfechaAprobacion
	);
	...
	$stmt = $connection->prepare("SELECT COUNT(*) AS allcount FROM pagos INNER JOIN alumnos ON pagos.idAlumno=alumnos.idAlumno INNER JOIN personas ON alumnos.idPersona=personas.idPersona INNER JOIN usuarios ON pagos.idUsuario=usuarios.idUsuario LEFT JOIN usuarios usuariosAprobo ON pagos.idUsuarioAprobo=usuariosAprobo.idUsuario");
	$stmt->execute();             
	$records = $stmt->fetch();
	$totalRecords = $records['allcount'];

	## Total number of records with filtering
	$stmt = $connection->prepare("SELECT COUNT(*) AS allcount FROM pagos INNER JOIN alumnos ON pagos.idAlumno=alumnos.idAlumno INNER JOIN personas ON alumnos.idPersona=personas.idPersona INNER JOIN usuarios ON pagos.idUsuario=usuarios.idUsuario LEFT JOIN usuarios usuariosAprobo ON pagos.idUsuarioAprobo=usuariosAprobo.idUsuario WHERE 1 ".$searchQuery);
    ...
    usuariosRegistro.username usuarioRegistro,
    pagos.nivelEscolar,
    pagos.gradoyGrupo,
    pagos.referencia,
    concepto,
    ...
    folio,
    idUsuarioAprobo,
    usuariosAprobo.username usuarioAprobo,
    fechaAprobacion
		FROM pagos 
        INNER JOIN alumnos ON pagos.idAlumno=alumnos.idAlumno 
        INNER JOIN personas ON alumnos.idPersona=personas.idPersona 
        INNER JOIN usuarios usuariosRegistro ON pagos.idUsuario=usuariosRegistro.idUsuario
        LEFT JOIN usuarios usuariosAprobo ON pagos.idUsuarioAprobo=usuariosAprobo.idUsuario
	...
	foreach($empRecords as $row){
		$data[] = array(
			...
			"folio"=>$row['folio'],
			"idUsuarioAprobo"=>$row['idUsuarioAprobo'],
